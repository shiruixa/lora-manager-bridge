...
**关掉标签页也不会丢结果。** 下载在服务端进行，关掉页面不会中断它；结果会由后台保留，通过**工具栏角标**（成功绿 / 失败红 + 数量）提示，并在你打开的任意 CivitAI 页面或工具栏弹窗里报出来 —— 不需要守着那个页面。
...
- **v1.4.0** — 关掉标签页也不会丢失下载结果：后台保存每个无人认领的结果，工具栏角标显示数量（成功绿/失败红），任意 CivitAI 页面或工具栏弹窗都会把它报出来
